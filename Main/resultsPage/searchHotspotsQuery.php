...
try
{
    ...
    if ($_GET['rating'] == 1 || $_GET['rating'] == 2 || $_GET['rating'] == 3 || $_GET['rating'] == 4 || $_GET['rating'] == 5) {
        $query = 'SELECT items.ID, items.Name, items.Address, items.Suburb, items.Latitude, items.Longitude,'.
        ...
        if (isset($_GET['locationSearch']) && isset($_GET['latitude']) && isset($_GET['longitude'])) {
            ...
            if ($_GET['locationSearch'] == 'on') {
                //add the condition of location to the query to search for hotspots within 1km of the user
    ...
    $stmt->bindValue(':name', '%'.htmlspecialchars($_GET['hotspotName']).'%');
